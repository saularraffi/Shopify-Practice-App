...
userRoutes.get("/debug/rest", async (req, res) => {
  ...
  console.log(getResponse.body.products[0].title);

  return res.status(200).json({ text: getResponse.body.products[0].title });
});

userRoutes.get("/debug/rest/manual", async (req, res) => {
  const getResponse = await axios.get(
    "https://saular-test-app.myshopify.com/admin/api/2024-01/products.json",
    {
      headers: {
        "Content-type": "application/json",
        "X-Shopify-Access-Token": "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",
      },
    }
  );

  return res.status(200).json({ text: getResponse.data.products[0].title });
});
